...
func TestDevInfo(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/rest/dev-status/latest/issue/summary" && q.Get("issueId") == "10042":
			io.WriteString(w, `{"summary":{"pullrequest":{"byInstanceType":{"GitLab":{"count":2}}},"branch":{"byInstanceType":{"GitLab":{"count":1},"GitHub":{"count":0}}},"repository":{"byInstanceType":{"GitLab":{"count":1}}},"build":{"byInstanceType":{"cloud-providers":{"count":1}}}}}`)
	...
	if len(got) != 5 || got[0].Name != "Fix login" || got[0].Branch != "issue/A-1 → main" || got[1].Status != "MERGED" ||
		got[2].Kind != "build" || got[2].Name != "CI #42" || got[2].Status != "FAILED" || got[2].Branch != "issue/A-1" ||
		got[3].Kind != "branch" || got[4].Kind != "commit" || got[4].Name != "a1b2c3d Fix login" || got[4].Status != "Ada" {
